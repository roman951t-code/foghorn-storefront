...
// would silently break error/replay reporting in production. (Verified this
// still holds 2026-07-22: tried dropping it against a real prod build with
// this app's actual Sentry DSN — couldn't get a clean before/after signal
// through automated tooling since no reliable way to force a genuine
// Sentry-captured error through it, so left as-is rather than gamble with
// production error monitoring on an unproven change.)
//
// No bare 'https:' scheme-source here (unlike an earlier version of this
// policy) — grepped the whole app for any <script src="https://...">/
// next/script external src usage and found none beyond Stripe.js, which is
// already explicitly allowlisted below. A bare https: source would let any
// externally-hosted script execute if an XSS ever managed to inject a
// <script src> tag, so it's dropped now that nothing actually depends on it.
//
