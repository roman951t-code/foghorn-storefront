...
function buildLocalizedProductCopy(
	...
	const metaTitle = `${name} | ${categoryName}`;
...
const pageLocalizationBySlug: Record<string, Record<SupportedLocale, LocalizedPageCopy>> = {
	'privacy-policy': {
		en: {
			...
			metaDescription:
				'Learn how FOGHORNBAY collects, uses, and protects personal data during shopping and support interactions.',
		},
		uk: {
			...
			metaDescription:
				'Дізнайтеся, як FOGHORNBAY збирає, використовує та захищає персональні дані під час покупок і звернень у підтримку.',
		},
	},
	'cookie-policy': {
		en: {
			...
			metaDescription:
				'Read which cookies FOGHORNBAY uses, what they do, and how consent affects optional analytics and marketing cookies.',
		},
		uk: {
			...
			metaTitle: 'Політика cookie',
			metaDescription:
				'Перегляньте, які cookie використовує FOGHORNBAY, для чого вони потрібні та як згода впливає на необовʼязкові cookie.',
		},
	},
	'about-us': {
		en: {
			...
			metaTitle: 'About Us',
			metaDescription:
				'Learn about FOGHORNBAY values, customer service approach, and how we build a reliable shopping experience.',
		},
		uk: {
			title: 'Про нас',
			...
			metaTitle: 'Про нас',
			metaDescription:
				'Дізнайтеся про цінності FOGHORNBAY, підхід до сервісу клієнтів і принципи нашої роботи.',
		},
	},
	faq: {
		en: {
			...
			metaTitle: 'FAQ',
			metaDescription:
				'Find quick answers about delivery, returns, payments, and other common FOGHORNBAY questions.',
		},
		uk: {
			title: 'Часті питання',
			...
			metaTitle: 'Часті питання',
			metaDescription:
				'Перегляньте відповіді щодо доставки, повернення, оплати та інших поширених питань FOGHORNBAY.',
		},
	},
	guarantee: {
		en: {
			...
			metaTitle: 'Warranty',
			metaDescription:
				'Read FOGHORNBAY warranty terms and what to do when a product has a manufacturing defect.',
		},
		uk: {
			title: 'Гарантії',
			...
			metaTitle: 'Гарантії',
			metaDescription:
				'Ознайомтеся з гарантійними умовами FOGHORNBAY та алгоритмом дій при виявленні браку.',
		},
	},
	'public-offer': {
		en: {
			...
			metaDescription:
				'Review the public offer terms that apply to purchases made at FOGHORNBAY.',
		},
		uk: {
			...
			metaDescription:
				'Ознайомтеся з умовами публічної оферти, що діють для покупок у FOGHORNBAY.',
		},
	},
	'return-refund': {
		en: {
			...
			metaTitle: 'Returns and Refunds',
			metaDescription:
				'Learn FOGHORNBAY return timelines, eligibility requirements, and refund workflow.',
		},
		uk: {
			...
			metaTitle: 'Обмін та повернення',
			metaDescription: 'Дізнайтеся про строки, умови та порядок повернення коштів у FOGHORNBAY.',
		},
	},
	'shipping-terms': {
		en: {
			...
			content:
				'We deliver across Ukraine through popular carriers such as Nova Post and Ukrposhta.\n\nShipping cost and delivery time depend on your region and selected method. Typical delivery window is 1-3 business days.',
			metaTitle: 'Shipping Terms',
			metaDescription:
				'Check delivery methods, costs, and estimated shipping timelines for FOGHORNBAY orders.',
		},
		uk: {
			...
			metaTitle: 'Умови доставки',
			metaDescription: 'Перегляньте способи, строки та вартість доставки замовлень FOGHORNBAY.',
		},
	},
	terms: {
		en: {
			...
			metaDescription:
				'Read the terms governing website usage, orders, and customer responsibilities at FOGHORNBAY.',
		},
		uk: {
			...
			metaTitle: 'Умови та положення',
			metaDescription:
				'Ознайомтеся з правилами використання сайту, оформлення замовлень і відповідальністю сторін у FOGHORNBAY.',
		},
	},
};
...
async function main() {
	...
	// These 7 slugs already had bilingual copy prepared in pageLocalizationBySlug
	// (syncPageTranslations reads it below via buildLocalizedPageCopy), but no
	// base Page row was ever created for them — unlike privacy-policy/
	// cookie-policy above, getPageBySlug() always returned null, so every one
	// of these routes silently fell back to its page.tsx's hardcoded static
	// data file, most of which are single-locale (src/data/staticPages/*.ts —
	// e.g. about.ts, guarantees.ts are Ukrainian-only), regardless of the
	// requested locale. Seeding the base row here is what lets
	// syncPageTranslations actually reach and translate them.
	const additionalContentPages = [
		{
			slug: 'about-us',
			title: 'About Us',
			excerpt: 'Who we are and what service standards we follow.',
	...
	for (const contentPage of additionalContentPages) {
		await prisma.page.upsert({
	...
	await recalculateAllProductSortPrices();
...
// sort and price-slider max query against directly instead of resolving
// every matching row's discount window at request time (see
// src/utils/productEffectivePrice.ts, the single source of truth this
// mirrors). Run as one final bulk pass over every product rather than
// threaded through each individual create/update above: seed data touches
// pricing across several independent code paths (base creation, variants,
// any future discount-schedule seeding), and a single end-of-run pass stays
// correct regardless of how many of those paths exist, at the cost of one
// extra full-table pass — acceptable for a script that already reseeds the
// whole catalog every deploy (docs/cicd-pipeline.md §5).
async function recalculateAllProductSortPrices() {
	const now = new Date();
	const products = await prisma.product.findMany({
		select: {
			id: true,
	...
	console.log(`✅ Recalculated sortPrice for ${products.length} products.`);
}
