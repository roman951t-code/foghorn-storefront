...
const envSchema = z.object({
	...
	NEXT_PUBLIC_APP_URL: optionalUrlEnv('NEXT_PUBLIC_APP_URL'),
	...
	UPSTASH_REDIS_REST_URL: optionalUrlEnv('UPSTASH_REDIS_REST_URL'),
	...
	CACHE_REVALIDATE_ALERT_WEBHOOK_URL: optionalUrlEnv('CACHE_REVALIDATE_ALERT_WEBHOOK_URL'),
	OPS_ALERT_WEBHOOK_URL: optionalUrlEnv('OPS_ALERT_WEBHOOK_URL'),
...
if (parsedData.NODE_ENV === 'production' && !parsedData.EMAIL_FROM) {
	environmentWarnings.push(
		'EMAIL_FROM is not configured in production. Using fallback sender FOGHORNBAY <onboarding@resend.dev>.',
	);
}
