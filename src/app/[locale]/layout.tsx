...
import { DEFAULT_OG_IMAGE_PATH, SITE_METADATA, SITE_NAME } from '@/constants/site';
import {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
	...
	const localizedSiteMetadata = SITE_METADATA[resolvedLocale];
	const fullSiteTitle = `${localizedSiteMetadata.title} | ${SITE_NAME}`;
	const googleSiteVerification = Array.from(
		new Set([
			PUBLIC_GOOGLE_SITE_VERIFICATION,
			'xqSJtVwPjr9qQ9sMNBNLMjDXo64uekkV338CTUeCvac',
		]),
	);
	const bingSiteVerification =
		PUBLIC_BING_SITE_VERIFICATION || '6EF5C645B1636A231AFFAE1C2A740E78';
	const otherVerification = {
		...(bingSiteVerification
			? { 'msvalidate.01': bingSiteVerification }
			: {}),
		...(PUBLIC_YANDEX_SITE_VERIFICATION
			? { 'yandex-verification': PUBLIC_YANDEX_SITE_VERIFICATION }
			: {}),
	};
	...
	return {
		...
		title: {
			default: fullSiteTitle,
			template: `%s | ${SITE_NAME}`,
		},
		description: localizedSiteMetadata.description,
		keywords: [
			'online store',
			'product catalog',
			'electronics',
...
async function VisibleShell({
	...
	return (
		<>
			<JsonLd id='org-schema' nonce={cspNonce} data={orgJsonLd} />
			<Link
				href='#main-content'
				position='fixed'
				top='2'
		...
		'@graph': [
			{
				...
				logo: {
					'@type': 'ImageObject',
					url: `${APP_URL}/assets/images/logo/logo_big.png`,
					width: 380,
					height: 83,
				},
				sameAs: [] as string[],
