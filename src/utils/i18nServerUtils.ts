...
export async function getLocalizedMetadata(
	...
	const rawTitle = pagesT(`metadata.${pageKey}.title`);
	const normalizedTitle = normalizeMetadataTitle(rawTitle);
	const isHomePage = options?.pathname === '/';
	const title = isHomePage ? { absolute: buildAbsoluteTitle(normalizedTitle) } : normalizedTitle;
	const socialTitle = isHomePage ? buildAbsoluteTitle(normalizedTitle) : normalizedTitle;
