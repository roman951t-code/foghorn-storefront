...
// A sized fallback (matching the real IconButton's footprint) instead of the
// next/dynamic default of rendering nothing — without it, this icon pops
// into the header row from empty space the moment the client chunk loads,
// nudging AddToFavourite sideways.
const ShareProduct = dynamic(() => import('./ShareProduct'), {
	ssr: false,
	loading: () => <SkeletonCircle boxSize='9' />,
});
...
export default function AboutTab({
	...
	const shareI18nData = {
		...
		share: prodT('share'),
		shareTelegram: prodT('shareTelegram'),
		shareWhatsapp: prodT('shareWhatsapp'),
		shareX: prodT('shareX'),
		shareFacebook: prodT('shareFacebook'),
		shareViber: prodT('shareViber'),
	};
	...
	const shipmentOptions: DetailOption[] = [
		{ key: 'novaPost', label: checkoutT('shipment.novaPost'), icon: <FaTruck /> },
		...
		<VStack gap='8' colorPalette='gray' w='100%' alignItems='stretch'>
			...
			>
				...
				<VStack w='full' gap='2' alignItems='flex-start'>
					<Heading as='h2' size={{ base: '2xl', sm: '3xl' }} fontWeight='medium'>
						{product.name}
					</Heading>
					<Flex
						w='100%'
