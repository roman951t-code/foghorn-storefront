...
// Thumbs strip shows up to 4 at once (slidesPerView below) — loop mode
// needs more real slides than that to have anything to clone/cycle through.
const MIN_SLIDES_FOR_THUMBS_LOOP = 4;
...
function ThumbsSliderInternal({ images, productName, initialIndex = 0 }: ProductThumbsSliderProps) {
	...
	// Loop mode with too few real slides for the viewport can leave Swiper's
	// own DOM manipulation in a state React's reconciliation can't patch
	// (observed in production as an insertBefore HierarchyRequestError on
	// product pages). See ProductsSlider.tsx for the fuller writeup.
	const canLoopMain = resolvedGalleryImages.length > 1;
	const canLoopThumbs = resolvedGalleryImages.length > MIN_SLIDES_FOR_THUMBS_LOOP;
