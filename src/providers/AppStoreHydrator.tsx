	...
	useEffect(() => {
		...
		if (wasLogged === null) {
			if (!resolvedIsLoggedIn) {
				...
				// The wishlist merge also performs its authoritative fetch inside
				// the serialized wishlist operation queue.
				void Promise.all([mergeGuestCart(), mergeGuestWish()]).then(fetchServerCart);
			}
