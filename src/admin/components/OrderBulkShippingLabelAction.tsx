...
const buildAddressLines = (order: ShippingLabelPayload): string[] => {
	const lines = [
...
export default function OrderBulkShippingLabelAction({ action, resource, records }: ActionProps) {
	const recordIds = useMemo(() => resolveRecordIds(records), [records]);
	const [payload, setPayload] = useState<BulkShippingLabelsPayload | null>(null);
	...
	useEffect(() => {
		...
		api.bulkAction({
			...
			.then((response) => {
				if (!isActive) return;
				setPayload((response.data.payload ?? null) as BulkShippingLabelsPayload | null);
			})
			.catch(() => {
				if (!isActive) return;
		...
		<Box variant='white' p='xxl' borderRadius='xl' boxShadow='sm' style={{ border: '1px solid #E2E8F0' }}>
			<Box display='flex' alignItems='center' justifyContent='space-between' mb='xl'>
				<Box>
					...
					<Text color='grey60'>{translateMessage('order-bulk-selected', { count: recordIds.length })}</Text>
				</Box>
				<Button
					variant='contained'
					color='primary'
			...
			{loading || !payload ? (
				...
				<Box style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
					{payload.orders.map((order, index) => {
						const recipientName =
							normalizeFullName(order.contactName, order.contactLastName) ??
							order.contactEmail ??
							order.contactPhone ??
							'-';
						...
						return (
							...
							>
								<Box style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
									...
									<Badge>{translateLabel(`status.${order.status}`)}</Badge>
								</Box>
								...
								<Box style={{ border: '1px dashed #334155', borderRadius: 12, padding: 12 }}>
									...
									<Text color='grey60'>{order.contactPhone ?? order.contactEmail ?? '-'}</Text>
									{addressLines.length ? (
