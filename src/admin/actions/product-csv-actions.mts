	...
	for (const product of products) {
		rows.push([
			...
			product.discountPrice?.toNumber?.() != null
				...
				: '',
			product.sortPrice?.toNumber?.() != null ? String(product.sortPrice.toNumber()) : '',
			product.currency ?? '',
...
export const importProductsCsv: ActionHandler<ActionResponse> = async (req, _res, context) => {
	...
	// Rows this import actually creates/updates — recomputed in one pass at
	// the end (see src/admin/lib/product-sort-price.mts) since this handler's
	// response shape doesn't carry record ids the way a normal AdminJS
	// new/edit action does, so the resource-mutation hook in
	// resources/index.mts can't pick these up on its own.
	...
	for (let i = 1; i < rows.length; i += 1) {
		...
		try {
			...
			if (existingId) {
				...
				affectedProductIds.push(existingId);
				if (stock != null) {
	...
	if (affectedProductIds.length > 0) {
		try {
			await recalculateProductSortPrices(prisma, affectedProductIds);
		} catch (error) {
			console.error('[admin-csv] Failed to recalculate product sort prices', error);
		}
	}
