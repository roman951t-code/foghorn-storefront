...
const resolveStoreAppUrl = () => {
	...
	// Mirrors the storeAppUrl resolution in src/admin/server.mts: never fall back to
	// localhost in production, where it can only ever resolve to an unreachable address
	// on the admin's own Render dyno and silently swallow every revalidation.
	return isProduction ? null : LOCAL_STORE_URL;
};

const buildRevalidateEndpointHint = () => {
	const base = (resolveStoreAppUrl() ?? LOCAL_STORE_URL).trim().replace(/\/+$/, '');
...
export const revalidateStorefrontCacheTags = async (tags: Array<string | null | undefined>) => {
	...
	const storeAppUrl = resolveStoreAppUrl();
	if (!storeAppUrl) {
		const details = {
	...
	try {
		const endpoint = new URL(REVALIDATE_PATH, storeAppUrl);
