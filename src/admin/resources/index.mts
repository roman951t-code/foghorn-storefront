...
const productEditAfterHook = async (
	...
	const localized = await localizedAfterHook(withGallerySynced, request, context);
	const editedProductId = collectMutationFieldValues('id', {
		response: localized as ActionResponse,
		request,
		context,
	});
	recalculateProductSortPrices(prisma, editedProductId).catch((error) =>
...
const extractProductIdsFromTags = (tags: string[]): string[] =>
	Array.from(
		new Set(
			tags
...
const runStorefrontTagRevalidation = async (
	...
	let tags: Array<string | null | undefined> = [];
	try {
	...
	try {
		...
		await recalculateProductSortPrices(prisma, extractProductIdsFromTags(cleanTags));
	} catch (error) {
		console.error('[admin-cache] Failed to recalculate product sort prices', error);
	}
};
...
export const resources = [
	{
		...
		options: {
			...
			properties: {
				...
				// Derived (see src/utils/productEffectivePrice.ts) — never editable
				// and fully hidden from the admin UI. It's an internal, precomputed
				// column the storefront's cheap/expensive sort queries directly;
				// surfacing it invited confusion (it can look "wrong" whenever it's
				// momentarily stale between a price edit and its recalculation).
				sortPrice: {
					type: 'currency',
					isVisible: { list: false, filter: false, show: false, edit: false },
					description: 'product-hint-sortPrice',
				},
				currency: {
					isRequired: true,
			...
			actions: {
				...
				new: {
					...
					after: async (response: any, request: any, context: any) => {
						...
						const localized = await localizedAfterHook(withGallery, request, context);
						const newProductId = collectMutationFieldValues('id', {
							response: localized,
							request,
							context,
						});
						recalculateProductSortPrices(prisma, newProductId).catch((error) =>
							console.error('[admin-cache] Failed to recalculate product sort prices', error),
						);
						return localized;
					},
