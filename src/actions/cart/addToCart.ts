...
export async function addToCart(item: AddToCartItem | AddToCartItem[]) {
	...
	try {
		...
		// uniqueItems, not from one another's results) — running them
		// concurrently instead of sequentially cuts this to one round-trip's
		// worth of latency.
		const [products, requestedVariants, defaultVariants] = await Promise.all([
		...
		Sentry.captureException(error, { tags: { cartAction: 'add-to-cart' } });
		return { success: false, message: cartT('cartUpdateFailed') };
	}
}
