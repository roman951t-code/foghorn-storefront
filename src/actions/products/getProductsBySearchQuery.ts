...
export async function getProductsBySearchQuery(
	...
	const getProducts = async (): Promise<ProductRow[]> => {
		...
		// promote popular/new/discounted/promotional items ahead of the rest,
		// instead of falling straight to the plain inStock/name ordering below.
		if (!orderBy) {
	...
	// totalCount/distinctCategories, products, and maxPriceAggregate are all
	// derived independently from whereClause — run them concurrently instead
	// of one after another to cut per-request latency.
	const [[totalCount, distinctCategories], products, maxPriceAggregate] = await Promise.all([
		prisma.$transaction([
