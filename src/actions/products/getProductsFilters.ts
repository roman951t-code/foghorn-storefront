...
export async function getSubcategoryFilters(subcategorySlug: string) {
	...
	// Independent of each other — run concurrently instead of sequentially.
...
// Not `'use cache'` — unlike the tag/search/subcategory filters above, this is
// scoped to one user's recently-viewed list, not a shared tag/query/category,
// so caching it would leak one user's facets to another.
export async function getViewedFilters(userId: string) {
	if (!userId) return [];

	const now = new Date();
	const publishedWhere = getPublishedProductWhere(now);
	const viewedByUser: Prisma.ProductWhereInput = {
		AND: [{ recentlyViewed: { some: { userId } } }, publishedWhere],
	};

	// Independent of each other — run concurrently instead of sequentially.
	const [brands, attributes] = await Promise.all([
		prisma.brand.findMany({
			where: { products: { some: viewedByUser } },
			select: { id: true, name: true, slug: true },
			orderBy: { name: 'asc' },
		}),
		prisma.productAttribute.findMany({
